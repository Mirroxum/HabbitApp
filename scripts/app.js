...
const page = {
	...
	header: {
		...
		progressPercent: document.querySelector('.progres__percent'),
		progressCoverBar: document.querySelector('.progres__cover-bar'),
	},
	content: {
...
function addDays(event) {
	...
	habbits = habbits.map((habbit) => {
		if (habbit.id === globalActiveHabbitId && comment) {
			return {
				...habbit,
...
function removeDays(daysIndex) {
	habbits = habbits.map((habbit) => {
		if (habbit.id === globalActiveHabbitId) {
			habbit.days.splice(daysIndex, 1);
		}
		return habbit;
	})
	rerender(globalActiveHabbitId);
